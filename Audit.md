...
- Of the 293 audited addins:
  ...
  - 113 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 138 have been transferred to the cake-contrib organisation
  - 118 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
