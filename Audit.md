...
- The analysis discovered 342 NuGet packages
  - 309 addins
  - 19 recipes
  - 11 modules
  - 3 uncategorized (see the 'Exceptions' section)
- Of the 339 categorized packages:
  - 324 were successfully audited
  - 15 were marked as deprecated
  - 0 could not be audited (see the 'Exceptions' section)

- Of the 324 audited addins:
  - 36 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 145 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  - 49 are using a custom icon hosted on a web site
  - 37 are embedding the cake-contrib icon (which is the current recommendation)
  - 6 are embedding a custom icon
  - 167 have been transferred to the cake-contrib organization
  - 180 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
