...
- The analysis discovered 308 NuGet packages
  - 280 addins
  - 16 recipes
  - 10 modules
  - 2 uncategorized (see the 'Exceptions' section)
- Of the 306 categorized packages:
  - 293 were successfully audited
  - 12 were marked as deprecated
  - 1 could not be audited (see the 'Exceptions' section)

- Of the 293 audited addins:
  - 51 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 111 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 138 have been transferred to the cake-contrib organisation
  - 114 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
