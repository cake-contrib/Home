...
- Of the 300 categorized packages:
  - 287 were successfully audited
  - 12 were marked as deprecated
  - 1 could not be audited (see the 'Exceptions' section)

- Of the 287 audited addins:
  - 70 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 75 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 119 have been transferred to the cake-contrib organisation
  - 70 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
