...
- The analysis discovered 299 NuGet packages
  - 271 addins
  - 16 recipes
  - 10 modules
  - 2 uncategorized (see the 'Exceptions' section)
- Of the 297 categorized packages:
  - 282 were successfully audited
  - 12 were marked as deprecated
  - 3 could not be audited (see the 'Exceptions' section)

- Of the 282 audited addins:
  - 75 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 66 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 115 have been transferred to the cake-contrib organisation
  - 60 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
