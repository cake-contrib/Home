...
- Of the 291 audited addins:
  ...
  - 82 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 124 have been transferred to the cake-contrib organisation
  - 78 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
