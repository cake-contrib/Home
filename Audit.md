...
- Of the 294 audited addins:
  ...
  - 131 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 140 have been transferred to the cake-contrib organisation
  - 135 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
