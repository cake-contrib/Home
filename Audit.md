...
- The analysis discovered 346 NuGet packages
  - 309 addins
  - 20 recipes
  - 11 modules
  - 6 uncategorized (see the 'Exceptions' section)
- Of the 340 categorized packages:
  - 309 were successfully audited
  - 16 were marked as deprecated
  - 15 could not be audited (see the 'Exceptions' section)

- Of the 309 audited addins:
  - 28 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 97 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  - 58 are using a custom icon hosted on a web site
  - 32 are embedding the cake-contrib icon (which is the current recommendation)
  - 47 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 7 are embedding a custom icon
  - 168 have been transferred to the cake-contrib organization
  - 195 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.AndroidAppManifest**: GetGithubMetadata: The response ended prematurely.

**Cake.AppCenter**: GetGithubMetadata: The response ended prematurely.

**Cake.AppVeyor**: GetGithubMetadata: The response ended prematurely.

**Cake.APT.Module**: GetGithubMetadata: The response ended prematurely.

**Cake.Args**: GetGithubMetadata: The response ended prematurely.

**Cake.ArgumentBinder**: GetGithubMetadata: The response ended prematurely.

**Cake.AssemblyInfoReflector**: GetGithubMetadata: The response ended prematurely.

**Cake.AWS.CodeDeploy**: GetGithubMetadata: The response ended prematurely.

**Cake.AWS.Lambda**: GetGithubMetadata: <!DOCTYPE html>

**Cake.AWS.Route53**: GetGithubMetadata: <!DOCTYPE html>

**Cake.AzCopy**: GetGithubMetadata: <!DOCTYPE html>

**Cake.AzureStorage**: GetGithubMetadata: <!DOCTYPE html>

**Cake.BitDiffer**: GetGithubMetadata: <!DOCTYPE html>

**Cake.Board.Asana**: GetGithubMetadata: <!DOCTYPE html>
...
**Cake.BuildSystems.Module**: GetGithubMetadata: <!DOCTYPE html>
