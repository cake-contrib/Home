...
- The analysis discovered 352 NuGet packages
  - 315 addins
  - 20 recipes
  - 11 modules
...
- Of the 326 audited addins:
  ...
  - 102 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  ...
  - 56 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 8 are embedding a custom icon
  - 177 have been transferred to the cake-contrib organization
  - 205 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.DocumentDb**: GetGithubMetadata: 500: Internal Server Error

**Cake.Issues.Testing**: AnalyzeNugetMetadata: This addin does not contain any decorated method.

**Cake.LightModeConsole.Module**: GetGithubMetadata: 500: Internal Server Error

**Cake.ProGet**: GetGithubMetadata: 500: Internal Server Error
