...
- The analysis discovered 345 NuGet packages
  - 311 addins
  - 20 recipes
  - 11 modules
  - 3 uncategorized (see the 'Exceptions' section)
- Of the 342 categorized packages:
  - 327 were successfully audited
  - 15 were marked as deprecated
  - 0 could not be audited (see the 'Exceptions' section)

- Of the 327 audited addins:
  - 35 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 144 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  - 48 are using a custom icon hosted on a web site
  - 42 are embedding the cake-contrib icon (which is the current recommendation)
  - 6 are embedding a custom icon
  - 169 have been transferred to the cake-contrib organization
  - 184 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
