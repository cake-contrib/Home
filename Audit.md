...
- The analysis discovered 370 distinct nuget packages.
- If you count all the versions of these addins, there's a grand total of 5174 packages on NuGet.org

## Statistics

- Of the 370 audited nuget packages:
  - 312 are addins
  - 11 are modules
  - 18 are recipes
  - 20 are marked as deprecated
  - 9 could not be audited (see the 'Exceptions' section)
- Of the 341 successfully analyzed packages:
  ...
  - 88 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 9 are embedding a custom icon
  - 186 have been transferred to the cake-contrib organization
  - 221 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.Bridge.DependencyInjection.Testing.Tests**: AnalyzeNugetMetadata: Could not find assembly 'xunit.v3.runner.common, Version=2.0.3.0, Culture=neutral, PublicKeyToken=8d05b1bb7a6fdb6c'. Either explicitly load this assembly using a method such as LoadFromAssemblyPath() or use a MetadataAssemblyResolver that returns a valid assembly.
