...
- The analysis discovered 339 NuGet packages
  ...
  - 18 recipes
  - 11 modules
  - 3 uncategorized (see the 'Exceptions' section)
- Of the 336 categorized packages:
  - 321 were successfully audited
  - 15 were marked as deprecated
  - 0 could not be audited (see the 'Exceptions' section)

- Of the 321 audited addins:
  - 37 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 144 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  - 49 are using a custom icon hosted on a web site
  - 35 are embedding the cake-contrib icon (which is the current recommendation)
  - 5 are embedding a custom icon
  - 166 have been transferred to the cake-contrib organization
  - 177 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.Cli**: AnalyzeNugetMetadata: This addin does not contain any decorated method.
