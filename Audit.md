...
- The analysis discovered 357 distinct nuget packages.
- If you count all the versions of these addins, there's a grand total of 4316 packages on NuGet.org

## Statistics

- Of the 357 audited nuget packages:
  - 302 are addins
  - 11 are modules
  - 18 are recipes
  - 18 are marked as deprecated
  - 8 could not be audied (see the 'Exceptions' section)
- Of the 331 successfully analyzed packages:
  - 29 are using the cake-contrib icon on the rawgit CDN (which has been shutdown since October 2019)
  ...
  - 62 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 9 are embedding a custom icon
  - 180 have been transferred to the cake-contrib organization
  - 210 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
