...
- The analysis discovered 319 NuGet packages
  - 290 addins
  - 16 recipes
  - 11 modules
  - 2 uncategorized (see the 'Exceptions' section)
- Of the 317 categorized packages:
  - 304 were successfully audited
  - 12 were marked as deprecated
  - 1 could not be audited (see the 'Exceptions' section)

- Of the 304 audited addins:
  - 42 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 139 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 58 are using a custom icon hosted on a web site
  - 16 are embedding the cake-contrib icon
  - 1 are embedding a custom icon
  - 146 have been transferred to the cake-contrib organisation
  - 157 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
