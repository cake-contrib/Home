...
- The analysis discovered 311 NuGet packages
  - 283 addins
  - 16 recipes
  - 10 modules
  - 2 uncategorized (see the 'Exceptions' section)
- Of the 309 categorized packages:
  - 296 were successfully audited
  - 12 were marked as deprecated
  - 1 could not be audited (see the 'Exceptions' section)

- Of the 296 audited addins:
  - 56 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 102 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 133 have been transferred to the cake-contrib organisation
  - 103 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
- The `NuGet package version` column indicates the version of the package that was used to perform the audit.
