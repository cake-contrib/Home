...
- Of the 293 audited addins:
  - 50 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 114 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 138 have been transferred to the cake-contrib organisation
  - 119 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
