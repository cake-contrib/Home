...
- Of the 328 audited addins:
  ...
  - 57 are embedding the cake-contrib icon (which is the current recommendation)
  - 6 are embedding a custom icon
  - 171 have been transferred to the cake-contrib organization
  - 186 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
