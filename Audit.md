...
- Of the 279 audited addins:
  - 130 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 9 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 114 have been transfered to the cake-contrib organisation
  - 21 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
