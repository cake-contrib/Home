...
- Of the 294 audited addins:
  - 42 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 137 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 71 are using a custom icon hosted on a web site
  - 0 are embedding the cake-contrib icon
  - 0 are embedding a custom icon
  - 142 have been transferred to the cake-contrib organisation
  - 146 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
