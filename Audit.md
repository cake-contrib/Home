...
- If you count all the versions of these addins, there's a grand total of 5504 packages on NuGet.org
...
- Of the 381 audited nuget packages:
  ...
  - 35 could not be audited (see the 'Exceptions' section)
- Of the 326 successfully analyzed packages:
  - 27 are using the cake-contrib icon on the rawgit CDN (which has been shutdown since October 2019)
  - 89 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  ...
  - 93 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 11 are embedding a custom icon
  - 180 have been transferred to the cake-contrib organization
  - 216 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.Addin.Analyzer**: GetGithubMetadata: The HTTP request failed with status code Unauthorized

**Cake.AliaSql**: GetGithubMetadata: The HTTP request failed with status code GatewayTimeout

**Cake.AppleSimulator**: GetGithubMetadata: The HTTP request failed with status code GatewayTimeout

**Cake.Asciidoctor**: GetGithubMetadata: The HTTP request failed with status code GatewayTimeout
...
**Cake.BuildSystems.Module**: GetGithubMetadata: The HTTP request failed with status code GatewayTimeout
...
**Cake.ClickOnce.Recipe**: GetGithubMetadata: The HTTP request failed with status code InternalServerError

**Cake.CodeQL.Cli**: GetGithubMetadata: The HTTP request failed with status code Unauthorized

**Cake.Console**: AnalyzeNugetMetadata: This addin does not contain any decorated method.

**Cake.Coveralls**: GetGithubMetadata: The HTTP request failed with status code Unauthorized
...
**Cake.Pulumi**: GetGithubMetadata: The HTTP request failed with status code Unauthorized
...
**Cake.VsCode**: GetGithubMetadata: The HTTP request failed with status code GatewayTimeout
...
**Cake.Yeoman**: GetGithubMetadata: The HTTP request failed with status code GatewayTimeout
