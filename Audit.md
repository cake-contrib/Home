...
- The analysis discovered 306 NuGet packages
  - 278 addins
  - 16 recipes
  - 10 modules
  - 2 uncategorized (see the 'Exceptions' section)
- Of the 304 categorized packages:
  - 291 were successfully audited
  - 12 were marked as deprecated
  - 1 could not be audited (see the 'Exceptions' section)

- Of the 291 audited addins:
  - 62 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 92 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 133 have been transferred to the cake-contrib organisation
  - 88 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
