...
- The analysis discovered 376 distinct nuget packages.
- If you count all the versions of these addins, there's a grand total of 5293 packages on NuGet.org

## Statistics

- Of the 376 audited nuget packages:
  - 314 are addins
  - 11 are modules
  - 21 are recipes
  - 20 are marked as deprecated
  - 10 could not be audited (see the 'Exceptions' section)
- Of the 346 successfully analyzed packages:
  ...
  - 53 are using a custom icon hosted on a web site
  ...
  - 226 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.Bridge.DependencyInjection.Testing.Tests**: AnalyzeNugetMetadata: Could not find assembly 'xunit.v3.runner.common, Version=3.0.1.0, Culture=neutral, PublicKeyToken=8d05b1bb7a6fdb6c'. Either explicitly load this assembly using a method such as LoadFromAssemblyPath() or use a MetadataAssemblyResolver that returns a valid assembly.
...
**Cake.Generator.Core.BenchmarkSuite**: AnalyzeNugetMetadata: Could not find assembly 'BenchmarkDotNet, Version=0.15.2.0, Culture=neutral, PublicKeyToken=aa0ca2f9092cefc4'. Either explicitly load this assembly using a method such as LoadFromAssemblyPath() or use a MetadataAssemblyResolver that returns a valid assembly.
