...
- Of the 287 audited addins:
  ...
  - 74 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 116 have been transferred to the cake-contrib organisation
  - 69 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
