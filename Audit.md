...
- Of the 343 categorized packages:
...
- Of the 328 audited addins:
  ...
  - 139 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  - 45 are using a custom icon hosted on a web site
  - 51 are embedding the cake-contrib icon (which is the current recommendation)
  - 6 are embedding a custom icon
  - 170 have been transferred to the cake-contrib organization
  - 185 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
