...
- Of the 278 audited addins:
  ...
  - 42 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 113 have been transfered to the cake-contrib organisation
  - 38 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
