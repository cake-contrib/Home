...
- Of the 329 audited addins:
  ...
  - 26 are embedding the cake-contrib icon (which is the current recommendation)
  - 61 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 9 are embedding a custom icon
  - 179 have been transferred to the cake-contrib organization
  - 208 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.Liquibase**: GetGithubMetadata: Server Error
