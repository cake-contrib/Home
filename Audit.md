...
- Of the 326 audited addins:
  ...
  - 123 are using the cake-contrib icon on the jsDelivr CDN (which was our preferred CDN to replace rawgit)
  - 44 are using a custom icon hosted on a web site
  - 66 are embedding the cake-contrib icon (which is the current recommendation)
  - 6 are embedding a custom icon
  - 170 have been transferred to the cake-contrib organization
  - 189 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
