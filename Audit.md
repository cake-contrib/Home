...
- Of the 331 categorized packages:
...
- Of the 315 audited addins:
  ...
  - 42 are using a custom icon hosted on a web site
  - 60 are embedding the cake-contrib icon (which is the current recommendation)
  - 11 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 6 are embedding a custom icon
  - 175 have been transferred to the cake-contrib organization
  - 190 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
