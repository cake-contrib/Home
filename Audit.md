...
- The analysis discovered 303 NuGet packages
  - 275 addins
  - 16 recipes
  - 10 modules
  - 2 uncategorized (see the 'Exceptions' section)
- Of the 301 categorized packages:
  - 286 were successfully audited
  - 12 were marked as deprecated
  - 3 could not be audited (see the 'Exceptions' section)

- Of the 286 audited addins:
  - 71 are using the cake-contrib icon on the rawgit CDN (which will be shutdown in October 2019)
  - 73 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 116 have been transferred to the cake-contrib organisation
  - 67 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.Board**: This addin seems to be referencing neither Cake.Core nor Cake.Common.

**Cake.Board.Abstractions**: This addin seems to be referencing neither Cake.Core nor Cake.Common.
