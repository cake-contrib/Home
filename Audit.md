...
- Of the 323 audited addins:
  ...
  - 34 are embedding the cake-contrib icon (which is the current recommendation)
  - 49 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 7 are embedding a custom icon
  - 176 have been transferred to the cake-contrib organization
  - 201 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.VulnerabilityScanner**: AnalyzeNugetMetadata: Could not find type 'System.CodeDom.Compiler.GeneratedCodeAttribute' in assembly 'C:\projects\cake-addindiscoverer\artifacts\Publish\System.Runtime.dll'.
