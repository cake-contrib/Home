...
- The analysis discovered 351 NuGet packages
  - 314 addins
  - 20 recipes
  - 11 modules
  - 6 uncategorized (see the 'Exceptions' section)
- Of the 345 categorized packages:
...
- Of the 326 audited addins:
  ...
  - 29 are embedding the cake-contrib icon (which is the current recommendation)
  - 54 are embedding one of the "fancy" cake-contrib icons (which is also recommended)
  - 8 are embedding a custom icon
  - 176 have been transferred to the cake-contrib organization
  - 204 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
...
**Cake.ArgumentBinder**: GetGithubMetadata: 500: Internal Server Error
...
**Cake.Ftp**: GetGithubMetadata: 500: Internal Server Error
