...
- Of the 318 audited addins:
  ...
  - 23 are embedding the cake-contrib icon (which is the current recommendation)
  - 3 are embedding a custom icon
  - 150 have been transferred to the cake-contrib organisation
  - 169 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
