...
- Of the 294 audited addins:
  ...
  - 108 are using the cake-contrib icon on the jsDelivr CDN (which is our preferred CDN to replace rawgit)
  - 141 have been transferred to the cake-contrib organisation
  - 109 have replaced the obsolete `licenseUrl` with proper license metadata (see the `Additional audit results` section below for details)
